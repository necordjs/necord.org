...
export class AppService {
    ...
    public onConnect(@Context() [node]: NodeManagerContextOf<'connect'>): void {
        this.logger.log(`Node: ${node.options.id} Connected`);
    }

    @OnLavalinkManager('playerCreate')
    public onPlayerCreate(@Context() [player]: LavalinkManagerContextOf<'playerCreate'>): void {
        this.logger.log(`Player created at ${player.guildId}`);
    }
}
```

### LavalinkManager Events {#lavalinkmanager-events}
...
- `@necord/lavalink` have snippets to access the lavalink-client methods. Each manager is available under its own injection token, exposed through a dedicated decorator.
...
export class AppService {
    public constructor(
        ...
        @InjectManagerUtils() private readonly managerUtils: ManagerUtils,
    ) {}
}
```

| Decorator                  | Token                   | Manager Property (Will access to) | Description      |
|----------------------------|-------------------------|-----------------------------------|------------------|
| `@InjectLavalinkManager()` | `LAVALINK_MANAGER`      | `lavalinkManager`                 | Lavalink Manager |
| `@InjectNodeManager()`     | `LAVALINK_NODE_MANAGER` | `lavalinkManager.nodeManager`     | Node Manager     |
| `@InjectManagerUtils()`    | `LAVALINK_UTILS`        | `lavalinkManager.utils`           | Manager Utils    |
| `@InjectPlayerStore()`     | `PlayerStore`           | `autoResume.playerStore`          | Player Store     |

`PlayerManagerService` and `NecordLavalinkService` are classes owned by `@necord/lavalink`, so they are injected normally, without a decorator:
...
Injecting the managers by class type is no longer supported:
...
`lavalink-client` ships both a CommonJS and an ESM build. An application that loads it in a different module format than `@necord/lavalink` gets two distinct `LavalinkManager` class objects — same name, same shape, different identity. Nest resolves providers by reference, so injecting by class type failed with `UnknownDependenciesException` in every ESM application:

```
Nest can't resolve dependencies of the AppService (?). Please make sure that the
argument NodeManager at index [0] is available in the AppModule module.
```

The tokens are created with `Symbol.for()`, which resolves through the global symbol registry by key instead of by reference, so they stay valid no matter how many times the package is loaded.
...
export class SourceAutocompleteInterceptor extends AutocompleteInterceptor {
    public transformOptions(interaction: AutocompleteInteraction): Promise<void> {
        ...
        const choices = focused.name === 'source' ? [DefaultSources.soundcloud] : [];
