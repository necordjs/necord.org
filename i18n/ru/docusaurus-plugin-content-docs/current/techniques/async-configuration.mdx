...
Приведенная выше конструкция создает экземпляр класса `NecordConfigService` внутри `NecordModule`, используя его для создания требуемого объекта опций. The class must expose a `createNecordOptions()` method returning `NecordModuleOptions`; `NecordModule` calls this method on the supplied class.
